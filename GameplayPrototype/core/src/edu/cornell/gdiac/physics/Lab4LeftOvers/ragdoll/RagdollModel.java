...
public class RagdollModel extends ComplexObstacle {
	...
	protected boolean createJoints(World world) {
		...
		int[] aArray = {1, 2, 3, 4, 5, 6, 7, 6, 7};
		int[] bArray = {0, 0, 0, 2, 3, 0, 0, 8, 9};
		float[] aXOffsets = {0,
				data.get( BODY_PARTS[partToAsset(2)] ).getFloat( 0 )/2,
				data.get( BODY_PARTS[partToAsset(3)] ).getFloat( 0 )/2,
				data.get( BODY_PARTS[partToAsset(4)] ).getFloat( 0 )/2,
				data.get( BODY_PARTS[partToAsset(5)] ).getFloat( 0 )/2,
				0, 0, 0, 0};
		float[] aYOffsets = {-1*data.get( BODY_PARTS[partToAsset(1)] ).getFloat( 1 )/2,
				0, 0, 0, 0,
				-1*data.get( BODY_PARTS[partToAsset(6)] ).getFloat( 1 )/2,
				-1*data.get( BODY_PARTS[partToAsset(7)] ).getFloat( 1 )/2,
				data.get( BODY_PARTS[partToAsset(8)] ).getFloat( 1 )/2,
				data.get( BODY_PARTS[partToAsset(9)] ).getFloat( 1 )/2};
		float[] bXOffsets = {0,
				-1*data.get( BODY_PARTS[partToAsset(2)] ).getFloat( 0 )/2,
				data.get( BODY_PARTS[partToAsset(3)] ).getFloat( 0 )/2,
				data.get( BODY_PARTS[partToAsset(8)] ).getFloat( 1 )/2,
				data.get( BODY_PARTS[partToAsset(9)] ).getFloat( 1 )/2,
				-1*data.get( BODY_PARTS[partToAsset(6)] ).getFloat( 0 ),
				data.get( BODY_PARTS[partToAsset(7)] ).getFloat( 0 ),
				0, 0};
		float[] bYOffsets = {data.get( BODY_PARTS[partToAsset(1)] ).getFloat( 1 )/2,
				data.get( BODY_PARTS[partToAsset(2)] ).getFloat( 1 ),
				data.get( BODY_PARTS[partToAsset(3)] ).getFloat( 1 ),
				0, 0,
				data.get( BODY_PARTS[partToAsset(6)] ).getFloat( 1 )/2,
				data.get( BODY_PARTS[partToAsset(7)] ).getFloat( 1 )/2,
				-1*data.get( BODY_PARTS[partToAsset(8)] ).getFloat( 1 )/2,
				-1*data.get( BODY_PARTS[partToAsset(9)] ).getFloat( 1)/2};

		for (int i = 0; i < 9; i++){
			RevoluteJointDef jointDef = new RevoluteJointDef();
			jointDef.bodyA = bodies.get(aArray[i]).getBody();
			jointDef.bodyB = bodies.get(bArray[i]).getBody();
			jointDef.localAnchorA.set(aXOffsets[i], aYOffsets[i]);
			jointDef.localAnchorB.set(bXOffsets[i], bYOffsets[i]);
