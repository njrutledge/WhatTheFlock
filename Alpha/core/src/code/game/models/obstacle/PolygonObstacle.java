 ...
 * game objects, but it will allow you to draw arbitrary shapes.
 *
 * Be careful with modifying this file.  Even if you have had computer
 * graphics, there are A LOT of subtleties in handling the game of
 * polygons.
 *
...
public class PolygonObstacle extends SimpleObstacle {
	...
	/** Shape information for this game object */
	 ...
	 * are measured in game units.  They tile the image according
	...
	private void initShapes(float[] points) {
		...
		// Allocate space for game triangles.
	...
	public void draw(GameCanvas canvas) {
		if (region != null) {
			//canvas.draw(region,Color.WHITE,0,0,0,0,getAngle(),.5f,.5f);
			//canvas.draw(region,getX(),getY());
			canvas.draw(texture,Color.WHITE, getX(),getY(),getX()*drawScale.x,getY()*drawScale.y,getAngle(),1.5f,1.5f);
			//canvas.draw(region,Color.WHITE,0,0,getX(), getY(), getAngle(),1,1);
		}
	}
