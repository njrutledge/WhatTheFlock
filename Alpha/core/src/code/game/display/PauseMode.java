...
public class PauseMode implements Screen, InputProcessor, ControllerListener {
    ...
    private FilmStrip cont;
    /** Restart button */
    private Texture restartTexture;
    private FilmStrip restart;
    /** Options button */
     ...
     * 0 for nothing,
     * 1 for continue pressed down, 11 for continue selected
     * 2 for restart pressed down, 12 for restart selected
    ...
    private final float BACKGROUND_SCALE = 0.65f;
    /** Scale for buttons */
    private final float BUTTON_SCALE = .66f;
    /** Height of all buttons */
    private final float BUTTON_HEIGHT = 37;
    /** Standard width of Start Button */
    private static int CONT_WIDTH = 222;
    /** Standard width of HTP Button */
    private static int RESTART_WIDTH = 474;
    ...
    private float buttonsCenterX;
    private float contCenterY;
    private float restartCenterY;
    private float optionsCenterY;
    private float quitCenterY;
     ...
     */
    public PauseMode(AssetDirectory assets, GameCanvas canvas) {
        this.canvas  = canvas;
        ...
        menuTexture = internal.getEntry( "background:pause", Texture.class );
        menuTexture.setFilter( Texture.TextureFilter.Linear, Texture.TextureFilter.Linear );
        contTexture = internal.getEntry( "ui:pause:continue", Texture.class );
        cont = new FilmStrip(contTexture, 1, 2);
        restartTexture = internal.getEntry( "ui:pause:restart", Texture.class );
        restart = new FilmStrip(restartTexture, 1, 2);
        optionsTexture = internal.getEntry( "ui:pause:options", Texture.class );
        options = new FilmStrip(optionsTexture, 1, 2);
        quitTexture = internal.getEntry( "ui:pause:quit", Texture.class );
    ...
    private void draw() {
        ...
        canvas.draw(menuTexture, Color.WHITE, menuTexture.getWidth()/2, menuTexture.getHeight()/2,
                bkgCenterX, bkgCenterY, 0, BACKGROUND_SCALE * scale, BACKGROUND_SCALE * scale);

        //continue
        canvas.draw(cont, Color.WHITE, cont.getRegionWidth()/2, cont.getRegionHeight()/2,
                buttonsCenterX, contCenterY, 0, BUTTON_SCALE * scale, BUTTON_SCALE * scale);
        //restart
        canvas.draw(restart, Color.WHITE, restart.getRegionWidth()/2, restart.getRegionHeight()/2,
                buttonsCenterX, restartCenterY, 0, BUTTON_SCALE * scale, BUTTON_SCALE * scale);
        //options
        canvas.draw(options, Color.WHITE, options.getRegionWidth()/2, options.getRegionHeight()/2,
                buttonsCenterX, optionsCenterY, 0, BUTTON_SCALE * scale, BUTTON_SCALE * scale);
        //quit
        canvas.draw(quit, Color.WHITE, quit.getRegionWidth()/2, quit.getRegionHeight()/2,
    ...
    public void resize(int width, int height) {
        ...
        contCenterY = 2.8f*height/5;
        restartCenterY = contCenterY - 2f * BUTTON_HEIGHT * BUTTON_SCALE;
        optionsCenterY = restartCenterY - 2f * BUTTON_HEIGHT * BUTTON_SCALE;
        quitCenterY = optionsCenterY - 2f * BUTTON_HEIGHT * BUTTON_SCALE;
        heightY = height;
    }
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        // TODO: Fix scaling
        if(overButton(CONT_WIDTH, screenX, screenY, buttonsCenterX, contCenterY)){
            pressState = 1;
        }else if(overButton(RESTART_WIDTH, screenX, screenY, buttonsCenterX, restartCenterY)){
            pressState = 2;
